...
void Util::InitSpecific()
{
  _vectorString.reserve(ReservationSize);
  _vectorGameObjectP.reserve(ReservationSize);
  _vectorItemComponentP.reserve(ReservationSize);
  _vectorUnsignedChar.reserve(ReservationSize);
  _vectorPosition.reserve(ReservationSize);
  _string.reserve(ReservationSize);
}
  ...
  while (true)
  {
    ...
    for (size_t i = 0; i < toSearch.length(); i++)
    {
      if (toSearch[i] == delim)
      {
        ...
        _vectorString.push_back(before);
    ...
    if (!found)
    {
      if (_vectorString.empty())
      {
        _vectorString = { str };
      }
      else
      {
        _vectorString.push_back(toSearch);
      }
...
//
// ===========================================================================
std::vector<std::string> Util::DecodeMap(const CM& map)
{
  _vectorString.clear();
  ...
  for (auto& block : map)
  {
    ...
    _vectorString.push_back(line);
  }

  return _vectorString;
}

std::string Util::DecodeString(const CS& str)
{
  _string.clear();
...
bool Util::IsObjectInRange(GameObject* checker,
                            GameObject* checked,
                            int range)
{
...
bool Util::IsObjectInRange(const Position& posToCheckFrom,
...
std::vector<GameObject*> Util::FindObjectsInRange(GameObject* aroundWho,
                                                   const std::vector<std::vector<std::unique_ptr<GameObject>>>& where,
                                                   int range)
{
  _vectorGameObjectP.clear();

  if (aroundWho == nullptr)
  {
    return _vectorGameObjectP;
  }
  ...
  for (int x = lx; x <= hx; x++)
  {
    for (int y = ly; y <= hy; y++)
    {
      ...
      {
        _vectorGameObjectP.push_back(go);
      }
    }
  }
  ...
  for (int x = lx; x <= hx; x++)
  {
    for (int y = ly; y <= hy; y++)
    {
      ...
      for (auto& i : where)
      {
        ...
        {
          _vectorGameObjectP.push_back(i.get());
        }
      }
    }
  }
...
std::string Util::Base64_Encode(unsigned char const* bytes_to_encode,
                                unsigned int in_len)
{
  _string.clear();
  ...
  while (in_len--)
  {
    ...
    if (i == 3)
    {
      ...
      for(i = 0; (i < 4) ; i++)
      {
        _string += Strings::Base64Chars[char_array_4[i]];
      }
  ...
  if (i)
  {
    ...
    for (j = 0; (j < i + 1); j++)
    {
      _string += Strings::Base64Chars[char_array_4[j]];
    }
...
std::string Util::Base64_Decode(const std::string& encoded_string)
{
...
std::vector<unsigned char> Util::ConvertStringToBytes(const std::string& encodedStr)
{
  _vectorUnsignedChar.clear();

  for (auto& c : encodedStr)
  {
      _vectorUnsignedChar.push_back(c);
  }

  return _vectorUnsignedChar;
}

bool Util::CheckLimits(const Position& posToCheck, const Position& limits)
{
...
std::vector<Position> Util::BresenhamLine(int sx, int sy, int ex, int ey)
{
  int x1 = sx;
  int y1 = sy;
  int x2 = ex;
  ...
  bool cond = (_vectorPosition[0].X == sx && _vectorPosition[0].Y == sy);
  if (!cond)
  {
    std::reverse(_vectorPosition.begin(), _vectorPosition.end());
  }
...
std::vector<Position> Util::BresenhamLine(const Position& start,
                                          const Position& end)
{
  return BresenhamLine(start.X, start.Y, end.X, end.Y);
}

bool Util::IsInsideMap(const Position& pos,
...
int Util::BlockDistance(int x1, int y1, int x2, int y2)
{
   return abs(y2 - y1) + abs(x2 - x1);
}

int Util::BlockDistance(const Position& from, const Position& to)
{
   return abs(to.Y - from.Y) + abs(to.X - from.X);
}

float Util::LinearDistance(int x1, int y1, int x2, int y2)
{
  float d = std::sqrt(std::pow(x2 - x1, 2) + std::pow(y2 - y1, 2));
  return d;
}

float Util::LinearDistance(const Position& s, const Position& e)
{
  return LinearDistance(s.X, s.Y, e.X, e.Y);
}

double Util::Log2(uint64_t n)
{
  //
...
std::vector<Position> Util::GetEightPointsAround(const Position& pos,
                                                 const Position& mapSize)
{
  _vectorPosition.clear();
...
std::vector<Position> Util::GetScreenRect(int x1, int y1, int x2, int y2)
{
  _vectorPosition.clear();
...
std::vector<Position> Util::GetScreenRectPerimeter(int x1,
                                                   int y1,
                                                   int x2,
                                                   int y2,
                                                   bool includeCorners)
{
  ...
  for (int x = lx; x <= hx; x++)
  {
    for (int y = ly; y <= hy; y++)
    {
      ...
      if (cond)
      {
        _vectorPosition.push_back(Position(x, y));
      }
    }
  }
...
std::vector<Position> Util::GetScreenRectAroundPoint(int pointX,
                                                     int pointY,
                                                     int rangeX,
                                                     int rangeY)
{
...
std::vector<Position> Util::GetRectAroundPoint(int pointX,
                                               int pointY,
                                               int rangeX,
                                               int rangeY,
                                               const Position& mapSize)
...
std::vector<Position> Util::GetPerimeter(int x,
                                         int y,
                                         int w,
                                         int h,
                                         bool includeCorners)
{
  ...
  for (int i = x1; i <= x2; i++)
  {
    for (int j = y1; j <= y2; j++)
    {
      ...
      if (cond)
      {
        _vectorPosition.push_back(Position(i, j));
      }
    }
  }
...
std::vector<Position> Util::GetPerimeterAroundPoint(int x,
                                                    int y,
                                                    int w,
                                                    int h,
                                                    bool includeCorners)
...
std::vector<GameObject*> Util::GetActorsInRange(GameObject* from, int range)
{
  _vectorGameObjectP.clear();
  ...
  for (auto& a : curLvl->ActorGameObjects)
  {
    if (a->PosX >= lx &&
        ...
        a->PosY <= hy)
    {
      _vectorGameObjectP.push_back(a.get());
    }
  }
...
std::vector<GameObject*> Util::GetContainersInRange(GameObject* from, int range)
{
  _vectorGameObjectP.clear();
  ...
  for (int x = lx; x <= hx; x++)
  {
    for (int y = ly; y <= hy; y++)
    {
      ...
      if (so != nullptr)
      {
        ...
        if (cc != nullptr)
        {
          _vectorGameObjectP.push_back(so);
        }
      }
    }
  }
...
///
std::vector<std::string> Util::RotateRoomLayout(const std::vector<std::string>& layout,
                                                RoomLayoutRotation r)
{
  _vectorString.clear();
  ...
  switch (r)
  {
    case RoomLayoutRotation::CCW_90:
    {
      ...
      {
        _vectorString.push_back(std::string(layout.size(), '_'));
      }

      int maxX = _vectorString.size() - 1;
    ...
    {
      _vectorString = layout;

      int maxY = _vectorString[0].length() - 1;
      ...
      for (size_t x = 0; x < layout.size(); x++)
      {
        for (size_t y = 0; y < layout[x].length(); y++)
        {
          _vectorString[x][maxY - y] = layout[x][y];
        }
      }

      std::reverse(_vectorString.begin(), _vectorString.end());
    }
    break;
...
int Util::RollDices(int numRolls, int diceSides)
{
  int totalDamage = 0;
...
int Util::GetExpForNextLevel(int curLvl)
{
  float p = std::pow(1.3f, curLvl)*20;
  return (int)p;
}

void Util::RecalculateWandStats(ItemComponent* wand)
{
  if (wand == nullptr)
  {
...
void Util::BlessItem(ItemComponent* item)
{
  if (item == nullptr)
  {
...
bool Util::Rolld100(int successChance)
{
  int dice = RNG::Instance().RandomRange(1, 101);
...
bool Util::WaitForMs(uint64_t delayMs, bool reset)
{
...
void Util::PrintVector(const std::string& title,
...
void Util::PrintLayout(const std::vector<std::string>& l)
{
  std::string dbg = "\n";
...
CharArray2D Util::StringsArray2DToCharArray2D(const StringsArray2D& map)
{
  _charArray2D.clear();
  ...
  for (auto& row : map)
  {
    ...
    _charArray2D.push_back(r);
  }

  return _charArray2D;
}

uint32_t Util::GetItemInventoryColor(const ItemData& data)
{
...
std::string Util::GenerateName(bool allowDoubleVowels,
                                bool canAddEnding,
                                const std::vector<std::string>& endings)
{
  ...
  if (addEnding == 0 && canAddEnding && !endings.empty())
  {
    ...
    if (!allowDoubleVowels)
    {
      if (_string[_string.length() - 1] == ending[0])
      {
        _string.pop_back();
      }
    }
...
std::string Util::ReplaceItemPrefix(const std::string& oldIdentifiedName,
                                     const std::vector<std::string>& anyOf,
                                     const std::string& replaceWith)
{
  _string = oldIdentifiedName;

  for (auto& s : anyOf)
  {
    size_t pos = _string.find(s);
    if (pos != std::string::npos)
    {
      _string = _string.replace(pos, s.length(), replaceWith);
      break;
    }
  }
...
std::string Util::GetCurrentDateTimeString()
{
  std::stringstream ss;
...
std::string Util::GetGameObjectDisplayCharacter(GameObject* obj)
{
  _string = "?";
  ...
  else
  {
    ...
    {
      _string = { (char)obj->Image };
    }
    else
    {
      if (!obj->ObjectName.empty())
      {
        _string = { obj->ObjectName[0] };
      }
    }
  }
...
size_t Util::FindLongestStringLength(const std::vector<std::string>& list)
{
  size_t res = 0;
...
void Util::LaunchProjectile(const std::vector<Position>& line,
                             char image,
...
void Util::LaunchProjectile(const Position& from,
                             const Position& to,
                             char image,
                             const uint32_t& fgColor,
                             const uint32_t& bgColor)
...
void Util::KnockBack(GameObject* sender,
                      GameObject* receiver,
...
void Util::ReapplyBonuses(GameObject* onWho, ItemComponent* item)
{
  onWho->UnapplyBonuses(item);
  onWho->ApplyBonuses(item);
}

std::string Util::ProcessTeleport(GameObject* target)
{
  _string.clear();
  ...
  if (actor != nullptr)
  {
    ...
    _string = StringFormat("%s suddenly disappears!", actor->ObjectName.data());
  }

  return _string;
}

std::pair<char, uint32_t> Util::GetProjectileImageAndColor(ItemComponent* weapon,
                                                             bool throwingFromInventory)
{
  ...
  if (weapon == nullptr)
  {
    ...
    _pairCharUint32 = { projectile, projColor };
    return _pairCharUint32;
  }
...
std::vector<GameObject*> Util::GetObjectsOnTheLine(const std::vector<Position>& line)
{
  _vectorGameObjectP.clear();
  ...
  for (size_t i = 1; i < line.size(); i++)
  {
    ...
    if (IsInsideMap(p, mapSize))
    {
      ...
      {
        _vectorGameObjectP.push_back(player);
      }

      auto actor = Map::Instance().GetActorAtPosition(p.X, p.Y);
      if (actor != nullptr)
      {
        _vectorGameObjectP.push_back(actor);
      }
      else
      {
...
GameObject* Util::GetFirstObjectOnTheLine(const std::vector<Position>& line)
{
...
std::pair<bool, std::string> Util::TryToDamageObject(GameObject* object,
                                                      GameObject* from,
                                                      int amount,
                                                      bool againstRes)
{
  _pairBoolString.first = false;
  ...
  if (object != nullptr)
  {
    ...
    if (dmgHere <= 0)
    {
      _pairBoolString.second = Util::StringFormat("%s seems unaffected!", object->ObjectName.data());;
    }
    else
    {
      object->ReceiveDamage(from, dmgHere, againstRes, false, false, !IsPlayer(from));
      _pairBoolString.first = true;
    }
  }

  return _pairBoolString;
}

std::string Util::TryToDamageEquipment(GameObject* actor,
                                        ...
                                        int damage)
{
  ...
  if (ec != nullptr)
  {
    ...
    _string = TryToDamageEquipment(actor, ic, damage);
  }
  else
  {
                                        ...
                                        int damage)
{
  ...
  {
    _string = StringFormat("%s breaks!", item->OwnerGameObject->ObjectName.data());
    item->Break(actor);
  }
...
int Util::CalculateDamageValue(GameObject* attacker,
                                GameObject* defender,
...
int Util::CalculateHitChanceMelee(GameObject* attacker,
                                    GameObject* defender)
{
...
int Util::CalculateHitChanceRanged(const Position& start,
                                     const Position& end,
                                     GameObject* user,
                                     ItemComponent* weapon,
                                     bool isThrowing)
...
int Util::GetTotalDamageAbsorptionValue(GameObject* who, bool magic)
{
  int res = 0;
...
std::vector<std::string> Util::ProcessPhysicalDamage(GameObject* who,
                                                      GameObject* from,
                                                      int& amount,
                                                      bool ignoreArmor)
{
  ...
  if (armorMsgs.empty())
  {
    ...
    _vectorString.push_back(StringFormat("%s => %s (%i)",
                                           fromImg.data(),
                                           whoImg.data(),
                                           amount));
  }
...
std::string Util::ProcessMagicalDamage(GameObject* who,
                                        GameObject* from,
                                        int& amount)
{
  _string.clear();
  ...
  {
    _string = StringFormat("You hit yourself for %i damage!", amount);
  }
  else
  {
    _string = StringFormat("%s => %s (%i)",
                           fromImg.data(),
                           whoImg.data(),
                           amount);
  }
...
void Util::ProcessManaShield(GameObject* who, int amount)
{
  if (who == nullptr)
  {
...
int Util::ProcessThorns(GameObject* who, int damageReceived)
{
  int dmgReturned = 0;
...
std::vector<std::string> Util::DamageArmor(GameObject* who,
                                            GameObject* from,
                                            int amount)
{
  _vectorString.clear();
  ...
  if (ec != nullptr)
  {
    ...
    if (armor != nullptr)
    {
      _vectorString.push_back(StringFormat("%s => [ (%i)", fromStr.data(), amount));
      ...
      if (armor->Data.Durability.Min().Get() <= 0)
      {
        if (IsPlayer(who))
        {
          _vectorString.push_back(StringFormat("%s breaks!", armor->OwnerGameObject->ObjectName.data()));
        }
  ...
  {
    _vectorString.push_back(msg);
  }

  return _vectorString;
}

Position Util::GetRandomPointAround(GameObject* user,
                                     ItemComponent* weapon,
...
std::vector<Position> Util::ProcessLaserAttack(GameObject* user,
                                                ItemComponent* weapon,
                                                const Position& end)
{
  _vectorPosition.clear();
  ...
  _vectorPosition = ProcessLaserAttack(user, { minDmg, maxDmg }, end);
                                                ...
                                                const Position& end)
{
  ...
  for (size_t i = 1; i < line.size(); i++)
  {
    ...
    _vectorPosition.push_back({ drawingPosX, drawingPosY });
...
void Util::DrawLaserAttack(const std::vector<Position>& line)
{
  for (auto& p : line)
  {
...
std::vector<Position> Util::GetAreaDamagePointsFrom(const Position& from, int range)
{
  _vectorPosition.clear();
  ...
  for (auto& p : perimeterPoints)
  {
    ...
    for (auto& point : line)
    {
      ...
      auto it = std::find_if(_vectorPosition.begin(),
                             _vectorPosition.end(),
                [&point](const Position& p) ->
                bool { return (p == point); });

      // If point was already added, skip it.
      if (it != _vectorPosition.end())
      {
        continue;
      }
      ...
      {
        _vectorPosition.push_back(cell->GetPosition());
      }
      else
      {
...
std::vector<ItemComponent*> Util::GetItemsWithBonus(GameObject* actor,
                                                     ItemBonusType bonus)
{
  _vectorItemComponentP.clear();

  if (actor == nullptr)
  {
    DebugLog("[WAR] Util::GetItemsWithBonus() actor is null!");
    return _vectorItemComponentP;
  }
  ...
  if (ec != nullptr)
  {
    for (auto& item : ec->EquipmentByCategory)
    {
      for (auto& e : item.second)
      {
        ...
        {
          _vectorItemComponentP.push_back(e);
        }
      }
    }
  }
...
Position Util::InvertDirection(const Position& dir)
{
  return { -dir.X, -dir.Y };
}

bool Util::IsPlayer(GameObject* obj)
{
  return (obj == &Application::Instance().PlayerInstance);
}

bool Util::CanBeSpawned(ItemComponent* ic)
{
...
size_t Util::CalculateItemHash(ItemComponent* item)
{
  if (item == nullptr)
  {
