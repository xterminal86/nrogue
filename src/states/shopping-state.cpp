...
void ShoppingState::DisplayPlayerInventory()
{
  ...
  int itemStringTotalLen =
      GetItemStringTotalLen(_playerRef->Inventory->Contents);
...
void ShoppingState::DisplayShopInventory()
{
  ...
  for (auto& item : _shopOwner->Items)
  {
    ...
    std::string costString = Util::StringFormat("%d $ ", cost);
...
size_t ShoppingState::GetItemStringTotalLen(
...
std::string ShoppingState::GetItemExtraInfo(ItemComponent* item)
{
  ...
  {
    extraInfo = Util::StringFormat("(%d)", item->Data.Amount);
  }
